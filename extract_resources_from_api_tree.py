...
def extract_resources_from_api_tree(hb_root, url_returned, token):
    ...
    for node in PreOrderIter(hb_root):
        ...
        # if the node's parent os highbond
            ...
            api_endpoint = url_returned + "/" + node.url
        ...
        # if node is child - call endpoint of child - base_url- base_url/parent/parent id/child
            ...
            child_resource_endpoint_list = build_child_url(
                url_returned, node.parent.name, node.url, node.parent.api_response
            )
            ...
            for child_resource_endpoint in child_resource_endpoint_list:
                ...
                if child_resp.status_code == 200:
                    ...
                    logging.info(
                        "Reponse code is 200 /successful " + child_resource_endpoint
                    )
                    ...
                    logging.info(
                        " The child base_url for the debugging purpose "
                        + child_resource_endpoint
                    )
                    ...
                    logging.error(
                        "The child resource which not able to make the api call is "
                        + child_resource_endpoint
                    )

                    # Loggingthe node names with API response code
                    logging.error(
                        node.name + "API response code " + str(child_resp.status_code)
                    )

            # Logging the node name as well as node depth for debugging purpose
            logging.info(
                "The node's depth from its root tree is: "
                + str(node.name)
                + "-"
                + str(node.depth)
            )
...
def build_child_url(base_url, parent_name, child_name, df_parent):
    ...
    if (
        df_parent is not None
        and isinstance(df_parent, pd.DataFrame)
        and not df_parent.empty
    ):
        ...
        for index, row in df_parent.iterrows():
            ...
            child_url_list.append(
                base_url + "/" + parent_name + "/" + row["id"] + child_name
            )
